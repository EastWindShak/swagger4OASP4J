...
export class ${variables.etoName?cap_first}DataGridComponent implements OnInit {
    ...
    columns: ITdDataTableColumn[] = [<#list pojo.fields as field>
      <#if field?has_next>
      {name: '${field.name}', label: this.getTranslation('${variables.component}.${variables.etoName}.columns.${field.name}')},
      <#else>
      {name: '${field.name}', label: this.getTranslation('${variables.component}.${variables.etoName}.columns.${field.name}')}
      </#if>
    </#list>];
    ...
    getTranslation(text: string): string {
        ...
        this.translate.onLangChange.subscribe(() => {
                me.columns.forEach( column => {
                    this.translate.get('${variables.component}.${variables.etoName}.columns.' + column.name).subscribe( (res) => {
                        column.label = res;
                    });
                });
