...
<#macro getNG2Type_Grid_Search>
  <#list model.properties as property>
      <md-input-container style="width:100%;">
        <input mdInput name ="${property.name}" type="${getType(OaspUtil.getOaspTypeFromOpenAPI(property, false, false))}" ngModel [placeholder]= "'${variables.component}datagrid.columns.${property.name}' | translate">
      </md-input-container>
   </#list>
</#macro>
...
<#macro getNG2Type_Add_Dialog>
  <#list model.properties as property>
    <md-input-container style="width:100%;">
        <input mdInput type="${getType(OaspUtil.getOaspTypeFromOpenAPI(property, false, false))}" name = "${property.name}" [placeholder]= "'${variables.component}datagrid.columns.${property.name}' | translate" [(ngModel)] = "items.${property.name}" required>
    </md-input-container>
   </#list>
</#macro>
